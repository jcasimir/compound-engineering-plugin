...
1. **Code Review (`/workflow:4-code-review`)**: If NOT already on the target branch (PR branch or requested branch), offer worktree for isolated review
2. **Feature Work (`/workflow:3-code`)**: Always ask if user wants parallel worktree or live branch work
...
The skill is automatically called from `/workflow:4-code-review` and `/workflow:3-code` commands:
